        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                <div class="flex items-center justify-center space-x-4">
                    ...
                    <div class="flex items-center">
                        <div class="w-8 h-8 {{ $sellerSale->current_step >= 2 ? ($sellerSale->current_step == 2 ? 'bg-blue-500' : 'bg-green-500') : 'bg-gray-300' }} text-white rounded-full flex items-center justify-center text-sm font-medium">
                            2
                        </div>
                        <span class="mr-2 text-sm font-medium {{ $sellerSale->current_step >= 2 ? ($sellerSale->current_step == 2 ? 'text-blue-600 dark:text-blue-400' : 'text-gray-900 dark:text-gray-100') : 'text-gray-500 dark:text-gray-400' }}">قرارداد فروش</span>
                    </div>
                    ...
                    <div class="flex items-center">
                        ...
                        <span class="mr-2 text-sm font-medium {{ $sellerSale->current_step >= 3 ? ($sellerSale->current_step == 3 ? 'text-blue-600 dark:text-blue-400' : 'text-gray-900 dark:text-gray-100') : 'text-gray-500 dark:text-gray-400' }}">کارمزد</span>
                    </div>
                    <div class="w-8 h-0.5 {{ $sellerSale->current_step >= 4 ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600' }}"></div>
                    <div class="flex items-center">
                        <div class="w-8 h-8 {{ $sellerSale->current_step >= 4 ? ($sellerSale->current_step == 4 ? 'bg-blue-500' : 'bg-green-500') : 'bg-gray-300' }} text-white rounded-full flex items-center justify-center text-sm font-medium">
                            4
                        </div>
                        <span class="mr-2 text-sm font-medium {{ $sellerSale->current_step >= 4 ? ($sellerSale->current_step == 4 ? 'text-blue-600 dark:text-blue-400' : 'text-gray-900 dark:text-gray-100') : 'text-gray-500 dark:text-gray-400' }}">پذیرش پیشنهاد</span>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    @if(!$contract)
                        ...
                        <!-- Send OTP Button -->
                        <form method="POST" action="{{ route('seller.sale.contract.otp', $auction) }}" class="mb-6">
                            @csrf
                            <button type="submit"
                    ...
                    @elseif($contract->status->value === 'otp_sent')
                        ...
                        <a href="{{ route('seller.sale.verify-contract', $auction) }}"
