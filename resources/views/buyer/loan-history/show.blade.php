...
<div class="container mx-auto px-4 py-8">
    <div class="mb-6">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('buyer.loan-history.index') }}"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white shadow rounded-lg p-6">
            ...
            <dl class="grid grid-cols-1 gap-4">
                ...
                <div>
                    <dt class="text-sm font-medium text-gray-500">شناسه مزایده</dt>
        ...
        <!-- User Bid Details -->
        <div class="bg-white shadow rounded-lg p-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">پیشنهاد شما</h2>
            @if($userBid)
                <dl class="grid grid-cols-1 gap-4">
                    <div>
                        <dt class="text-sm font-medium text-gray-500">مبلغ پیشنهادی</dt>
                        <dd class="mt-1 text-sm text-gray-900">{{ number_format($userBid->amount) }} تومان</dd>
                    </div>
                    <div>
                        <dt class="text-sm font-medium text-gray-500">وضعیت پیشنهاد</dt>
                        <dd class="mt-1">
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                @if($userBid->status->value === 'highest') bg-green-100 text-green-800
                                @elseif($userBid->status->value === 'accepted') bg-blue-100 text-blue-800
                                @elseif($userBid->status->value === 'outbid') bg-red-100 text-red-800
                                @else bg-gray-100 text-gray-800
                                @endif">
                                {{ $userBid->status->label() }}
                            </span>
                        </dd>
                    </div>
                    <div>
                        <dt class="text-sm font-medium text-gray-500">تاریخ پیشنهاد</dt>
                        <dd class="mt-1 text-sm text-gray-900">{{ $userBid->created_at->format('Y/m/d H:i') }}</dd>
                    </div>
                </dl>
            @else
                <p class="text-gray-500">پیشنهادی ثبت نشده است</p>
            @endif
        </div>
    </div>
