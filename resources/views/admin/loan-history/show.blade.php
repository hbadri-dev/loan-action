...
<div class="container mx-auto px-4 py-8">
        ...
        <div class="bg-white shadow rounded-lg p-6">
            ...
            <dl class="grid grid-cols-1 gap-4">
                <div>
                    ...
                    <dd class="mt-1 text-sm text-gray-900">{{ $auction->id }}</dd>
                </div>
                <div>
                ...
                <div>
                    <dt class="text-sm font-medium text-gray-500">فروشنده</dt>
                    <dd class="mt-1 text-sm text-gray-900">{{ $auction->creator->name ?? 'نامشخص' }}</dd>
                </div>
                <div>
                ...
                <div>
                    ...
                    <dd class="mt-1 text-sm text-gray-900">{{ $auction->created_at->format('Y/m/d H:i') }}</dd>
                </div>
                @if($auction->completed_at)
                    <div>
                        <dt class="text-sm font-medium text-gray-500">تاریخ تکمیل</dt>
                        <dd class="mt-1 text-sm text-gray-900">{{ $auction->completed_at->format('Y/m/d H:i') }}</dd>
                    </div>
                @endif
            </dl>
        </div>
    ...
    @if($acceptedBid)
        <div class="mt-6 bg-white shadow rounded-lg p-6">
            ...
            <dl class="grid grid-cols-1 gap-4">
                <div>
                    <dt class="text-sm font-medium text-gray-500">خریدار</dt>
                    <dd class="mt-1 text-sm text-gray-900">{{ $acceptedBid->buyer->name ?? 'نامشخص' }}</dd>
                </div>
                <div>
    ...
    <!-- Buyer Progress -->
    @if($buyerProgress->count() > 0)
        <div class="mt-6 bg-white shadow rounded-lg p-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">پیشرفت خریدار</h2>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">مرحله</th>
                            <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">وضعیت</th>
                            <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">تاریخ</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($buyerProgress as $progress)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                        @if($progress->is_completed) bg-green-100 text-green-800
                                        @else bg-yellow-100 text-yellow-800
                                        @endif">
                                        {{ $progress->is_completed ? 'تکمیل شده' : 'در انتظار' }}
                                    </span>
                                </td>
    ...
    @if($loanTransfer)
        <div class="mt-6 bg-white shadow rounded-lg p-6">
            ...
            <dl class="grid grid-cols-1 gap-4">
                <div>
                    <dt class="text-sm font-medium text-gray-500">فروشنده</dt>
                    <dd class="mt-1 text-sm text-gray-900">{{ $loanTransfer->seller->name ?? 'نامشخص' }}</dd>
                </div>
                <div>
                    <dt class="text-sm font-medium text-gray-500">خریدار</dt>
                    <dd class="mt-1 text-sm text-gray-900">{{ $loanTransfer->buyer->name ?? 'نامشخص' }}</dd>
                </div>
                <div>
                    <dt class="text-sm font-medium text-gray-500">کد ملی خریدار</dt>
                    <dd class="mt-1 text-sm text-gray-900">{{ $loanTransfer->national_id_of_buyer }}</dd>
                </div>
                <div>
