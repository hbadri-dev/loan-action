...
// OTP authentication routes for buyers/sellers
Route::prefix('auth')->name('auth.')->group(function () {
    // OTP request endpoint (API)
    Route::post('otp/request', [OtpRequestController::class, 'store'])
        ->name('otp.request');

    // OTP verify endpoint (API)
    Route::post('otp/verify', [OtpVerifyController::class, 'store'])
        ->name('otp.verify');
});

// OTP web authentication routes
Route::middleware('guest')->group(function () {
    // OTP login pages
    Route::get('otp/login', [OtpLoginController::class, 'show'])->name('otp.login');
    Route::post('otp/request', [OtpLoginController::class, 'requestOtp'])->name('otp.request.web');
    Route::get('otp/verify', [OtpLoginController::class, 'showVerify'])->name('otp.verify');
    Route::post('otp/verify', [OtpVerifyController::class, 'verifyWeb'])->name('otp.verify.web');
...
// Legacy Breeze routes (keep for admin)
Route::middleware('guest')->group(function () {
